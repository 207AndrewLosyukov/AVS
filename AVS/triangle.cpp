...
void OutTriangle(struct triangle *t, FILE* outputFile) {
    fprintf(outputFile, "Triangle:\n\tpoint1 (a) = ");
    fprintf(outputFile, "%d %d,\n\t", t->a.x, t->a.y);
    fprintf(outputFile, "point2 (b) = %d %d,\n\t", t->b.x, t->b.y);
    fprintf(outputFile, "point3 (c) = %d %d,\n\t", t->c.x, t->c.y);
    fprintf(outputFile, "Square = %f\n", SquareTriangle(t));
}

// Вычисление площади треугольника.
double SquareTriangle(struct triangle *t) {
    return (double)(ceil((1/2.0) * abs(t->a.x * (t->b.y - t->c.y) + t->b.x * (t->c.y - t->a.y) + t->c.x * (t->a.y - t->b.y))));
}