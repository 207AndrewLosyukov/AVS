 ...
 */
void generationTests::generateTest(int size) {
    FILE *inputFile = fopen("testGenerate.txt", "w");
 ...
 * @param y2
 */
void generationTests::generateRectangleCoordinates(int &x1, int &y1, int &x2, int &y2) {
    do {
 ...
 * @param y3
 */
void generationTests::generateTriangleCoordinates(int &x1, int &y1, int &x2, int &y2, int &x3, int &y3) {
    x1 = Random::next(0, 1000);
    y1 = Random::next(0, 1000);
 ...
 * @param r - радиус.
 */
void generationTests::generateCircleCoordinates(int &x, int &y, int &r) {
